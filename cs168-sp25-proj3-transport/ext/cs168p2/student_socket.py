...
class StudentUSocket(StudentUSocketBase):
  ...
  def handle_accepted_fin(self, seg):
    ...
    ## Start of Stage 7.2 ##
      ...
      if self.fin_ctrl.acks_our_fin(ack) and seg.FIN:
        self.set_pending_ack()
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    ## Start of Stage 2.2 ##
    if self.state in [ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2] and len(payload):
  ...
  def maybe_send(self):
    ...
    ## Start of Stage 4.3 ##
    remaining = min((snd.una |PLUS| snd.wnd) |MINUS| snd.nxt, len(self.tx_data))
    while remaining > 0:
      length = min(remaining, self.mss)
      payload = self.tx_data[0 : length]
      num_pkts += 1
      bytes_sent += length
      self.tx_data = self.tx_data[length:]
      remaining -= length
